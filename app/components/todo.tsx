    ...
    return (
        <li key={todo.date.toString()} className="py-2 px-4 flex justify-between items-center border-b border-gray-200">
            <div className="flex gap-2 items-center">
                ...
                <input type="checkbox" checked={todo.done} onChange={onChange} className="text-blue-500 rounded-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                <div className="overflow-wrap: brea max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
                <span className={todo.done ? 'line-through text-gray-400 break-words' : 'text-gray-800 break-words'}>{todo.text}</span>
                </div>
            </div>
            <div className="flex gap-2 items-center">
                ...
                <span className="text-xs text-gray-400">{new Date(todo.date).toDateString()}</span>
            </div>
        </li>
    )
}
